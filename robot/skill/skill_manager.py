...
class SkillManager:
    ...
    def __init__(self, robot_instance=None):
        ...
        logger.info(f"SkillManager initialized for robot: {robot_instance.namespace if robot_instance else 'Unknown'}")
    ...
    def execute_skill(self, skill_name: str, skill_args: Dict[str, Any] = None) -> bool:
        ...
        with self._lock:
            ...
            if skill_name not in self.skill_function:
                logger.error(f"Skill '{skill_name}' not found. Available skills: {list(self.skill_function.keys())}")
                return False
            
            # Check if another skill is already running
            if self.state == SkillState.RUNNING:
                logger.warning(f"Cannot execute skill '{skill_name}': another skill '{self.current_skill_name}' is running")
                return False
            ...
            logger.info(f"Skill '{skill_name}' started with args: {self.current_skill_args}")
            return True
    ...
    def _execute_skill_wrapper(self):
        ...
        try:
            ...
            if self.stop_flag.is_set():
                ...
                result = {"success": False, "message": "Skill execution was stopped", "data": None}
            
            # Validate result format
            if not isinstance(result, dict):
                logger.error(f"Skill '{skill_name}' returned invalid result format: {type(result)}")
                result = {"success": False, "message": "Invalid result format", "data": result}
            ...
            # Set final state
            if result["success"]:
                logger.info(f"Skill '{skill_name}' completed successfully: {result['message']}")
                self.state = SkillState.IDLE
            else:
                ...
                time.sleep(0.1)  # Brief delay to allow external reading of failure state
                self.state = SkillState.IDLE
