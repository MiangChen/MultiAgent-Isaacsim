...
def _handle_idle(robot, skill_name):
    ...
    # 获取存储的数据
    track_waypoint_list = robot.get_skill_data(skill_name, "track_waypoint_list", [])
    track_current_waypoint_index = robot.get_skill_data(skill_name, "track_current_waypoint_index", 0)
    ...
    if len(track_waypoint_list) > track_current_waypoint_index:
        ...
        return robot.form_feedback("processing", f"Starting navigation to waypoint {track_current_waypoint_index}", 10)
    else:
        # 没有新航点，继续等待
...
def _start_navigation_to_waypoint(robot, skill_name, goal_pos):
    ...
    track_current_waypoint_index = robot.get_skill_data(skill_name, "track_current_waypoint_index", 0)
    robot.node.get_logger().info(f"Tracking: Sending path request to waypoint {track_current_waypoint_index}: {goal_pos}")
...
def _handle_initializing(robot, skill_name):
    ...
    track_nav_result_future = robot.get_skill_data(skill_name, "track_nav_result_future")
    ...
    if track_nav_result_future is None:
        if track_nav_send_future.done():
            ...
            if goal_handle.accepted:
                ...
                robot.set_skill_data(skill_name, "track_nav_result_future", track_nav_result_future)
    ...
    if track_nav_result_future.done():
        ...
        if result.status == GoalStatus.STATUS_SUCCEEDED and result.result.path.poses:
            robot.node_controller_mpc.move_event.clear()
            robot.set_skill_data(skill_name, "track_nav_move_start_time", robot.sim_time)
...
def _handle_executing(robot, skill_name):
    ...
    if robot.node_controller_mpc.move_event.is_set():
        ...
        track_nav_move_start_time = robot.get_skill_data(skill_name, "track_nav_move_start_time", robot.sim_time)
        track_current_waypoint_index = robot.get_skill_data(skill_name, "track_current_waypoint_index", 0)
