...
def interpolate_path_linear(waypoints: List[Tuple[float, float]],
                            target_distance: float = 0.05) -> List[Tuple[float, float]]:
    """
...
def interpolate_path_spline(waypoints: List[Tuple[float, float]],
                            target_distance: float = 0.05,
                            smoothing: float = 0.0) -> List[Tuple[float, float]]:
    """
    样条插值路径，生成更平滑的轨迹
    ...
    try:
        ...
        # 样条插值
        tck, u = splprep([x_coords, y_coords], s=smoothing, k=min(3, len(waypoints) - 1))
        ...
        logger.debug(f"Spline interpolation: {len(waypoints)} -> {len(interpolated_points)} points")
        return interpolated_points
...
def interpolate_path_adaptive(waypoints: List[Tuple[float, float]],
                              target_distance: float = 0.05,
                              curvature_threshold: float = 0.1) -> List[Tuple[float, float]]:
    """
    自适应插值，在弯曲处增加更多点
    ...
    # 处理最后一段
    if len(waypoints) > 1:
        last_segment = interpolate_segment(waypoints[-2], waypoints[-1], target_distance)
...
def interpolate_segment(start: Tuple[float, float],
...
def interpolate_path_with_fixed_distance(waypoints: List[Tuple[float, float]],
                                         target_distance: float = 0.05,
