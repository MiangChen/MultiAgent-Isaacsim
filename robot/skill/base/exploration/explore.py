...
def _init_explore(robot, skill_name, kwargs):
    ...
    if not hasattr(robot, "node_controller_mpc") or not hasattr(
        robot, "node_planner_ompl"
    ):
        robot.skill_states[skill_name] = "FAILED"
    ...
    try:
        ...
        return robot.form_feedback(
            "processing",
            "Exploration waypoints planned, preparing to navigate to start point.",
            20,
        )
...
def _handle_executing2(robot, skill_name):
    ...
    try:
        ...
        target_prim = robot.get_skill_data(
            skill_name, "target_prim", "/TARGET_PRIM_NOT_SPECIFIED"
        )
        ...
        if start_time is None:
            ...
            return robot.form_feedback(
                "processing", "Starting exploration coverage...", 70
            )
