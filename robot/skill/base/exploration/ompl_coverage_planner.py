...
class OMPLCoveragePlanner:
    ...
    def set_validity_checker(self, polygon_coords: List[Tuple[float, float]], 
    ...
    def plan_stc_coverage(self, start_pos: Tuple[float, float], 
                         coverage_resolution: float = 0.5) -> Optional[List[Tuple[float, float]]]:
        ...
        try:
            ...
            if solved:
                ...
                logger.info(f"STC coverage planning succeeded with {len(waypoints)} waypoints")
                return waypoints
            else:
    ...
    def plan_bstar_coverage(self, start_pos: Tuple[float, float],
                           grid_resolution: float = 0.5) -> Optional[List[Tuple[float, float]]]:
        """使用B* 算法规划覆盖路径
        ...
        try:
            ...
            if solved:
                ...
                logger.info(f"B* coverage planning succeeded with {len(waypoints)} waypoints")
                return waypoints
            else:
    ...
    def plan_grid_coverage(self, bounds: List[Tuple[float, float]], 
                          spacing: float = 1.0,
...
def plan_ompl_coverage_waypoints(**kwargs) -> Optional[Path]:
    ...
    try:
        ...
        logger.info(f"OMPL coverage planning completed with {len(waypoints)} waypoints using {algorithm} algorithm")
        return path_msg
