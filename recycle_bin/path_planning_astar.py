...
class AStar:
    def __init__(
        ...
        if directions == "eight":
            ...
            raise ValueError(
                f"Unsupported directions: {directions}. Choose 'four', 'eight', '6', or '26'."
            )
        ...
        for obs_loc in obstacle_locations:
            ...
            for d in dist_directions:
                ...
                if (
                    ...
                    and 0 <= ny < y_size
                    and 0 <= nz < z_size
                    and self.map[nx, ny, nz]
                    == self.free_value  # Ensure it's free space
                    and distance_map[nx, ny, nz] == np.inf
                ):  # Ensure not already added/processed
        ...
        while queue:
            ...
            for d in dist_directions:
                ...
                if (
                    ...
                    and 0 <= ny < y_size
                    and 0 <= nz < z_size
                    and self.map[nx, ny, nz] == self.free_value
                ):  # Only propagate through free space
    ...
    def _is_valid(self, point: Tuple[int, int, int]) -> bool:
        ...
        return (
            ...
            and 0 <= y < self.y_size
            and 0 <= z < self.z_size
            and self.map[x, y, z] == self.free_value
    ...
    def _calculate_penalty(self, point: Tuple[int, int, int]) -> float:
        ...
        if distance == 0:  # Should not happen for valid free cells checked by _is_valid
            ...
            return self.penalty_factor / (
                distance**2 + 1.0
            )  # Or penalty_factor / distance, etc.
    ...
    def find_path(
        ...
        if not self._is_valid(start):
            raise ValueError(
                f"Start point {start} is invalid (out of bounds or obstacle in original map)"
            )
        if not self._is_valid(goal):
            raise ValueError(
                f"Goal point {goal} is invalid (out of bounds or obstacle in original map)"
            )
        ...
        if self.distance_map[start] == np.inf or self.distance_map[goal] == np.inf:
            print(
                f"Warning: Start ({start}) or Goal ({goal}) seem infinitely far from any obstacle. Check map connectivity."
            )
        ...
        initial_g = self._calculate_penalty(
            start
        )  # Cost to "reach" start from start, including its penalty
        heapq.heappush(
            open_set, (initial_g + self._heuristic(start, goal), initial_g, start)
        )
