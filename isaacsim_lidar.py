        ...
        if count % 10 == 0:  # Every 10 frames
            for drone in drones:
                if hasattr(drone.robot, "lidar_list") and drone.robot.lidar_list:
                    t_now = (
                        drone.robot.ros_manager.get_node().get_clock().now()
                    ...
                    if t_now:
                        ...
                        from simulation_utils_old.message_convert import (
                            create_pc2_msg,
                            create_image_msg,
                        )
                        ...
                        if hasattr(node, "publisher_dict"):
                            if "lfr_pc" in node.publisher_dict:
                                node.publisher_dict["lfr_pc"].publish(
                                    create_pc2_msg(header, pc_lfr)
                                )
                            if "ubd_pc" in node.publisher_dict:
                                node.publisher_dict["ubd_pc"].publish(
                                    create_pc2_msg(header, pc_ubd)
                                )
                            if "lfr_img" in node.publisher_dict:
                                node.publisher_dict["lfr_img"].publish(
                                    create_image_msg(
                                        header, drone.robot.lidar_list[0].get_depth()
                                    )
                                )
                            if "ubd_img" in node.publisher_dict:
                                node.publisher_dict["ubd_img"].publish(
                                    create_image_msg(
                                        header, drone.robot.lidar_list[1].get_depth()
                                    )
                                )
...
def main():
    ...
    print("\n  # Control via cmd_vel:")
    print(
        '  ros2 topic pub /autel_0/cmd_vel geometry_msgs/msg/Twist "{linear: {x: 1.0, y: 0.0, z: 0.0}}"'
    )
    ...
    run_simulation_loop(
        ...
        world,
        drones,
        camera_result.get("camera_instance"),
        camera_result.get("prim_path"),
        viewport_manager,
    )
