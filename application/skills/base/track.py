...
def track_callback(robot, skill_manager, msg):
    ...
    track_waypoint_list = skill_manager.get_skill_data(skill_name, "track_waypoint_list", [])
...
def _init_track(robot, skill_manager, skill_name, kwargs):
    ...
    if not robot.has_ros():
        ...
        skill_manager.skill_errors[skill_name] = "ROS not available - tracking requires ROS"
    ...
    skill_manager.set_skill_data(skill_name, "track_period", kwargs.get("track_period", 10))
    
    # Create subscription to target odometry
    target_topic = kwargs.get("target_topic", "/target_0/odom")
    track_callback_partial = functools.partial(track_callback, robot=robot, skill_manager=skill_manager)
...
def _handle_idle(robot, skill_manager, skill_name):
    ...
    if len(track_waypoint_list) > track_current_waypoint_index:
        ...
        return skill_manager.form_feedback("processing", "Waiting for target waypoints...", 5)
...
def _start_navigation_to_waypoint(robot, skill_manager, skill_name, goal_pos):
    ...
    skill_manager.set_skill_data(skill_name, "track_nav_send_future", track_nav_send_future)
    skill_manager.set_skill_data(skill_name, "track_nav_start_time", track_nav_start_time)
...
def _handle_initializing(robot, skill_manager, skill_name):
    ...
    track_nav_send_future = skill_manager.get_skill_data(skill_name, "track_nav_send_future")
    track_nav_start_time = skill_manager.get_skill_data(skill_name, "track_nav_start_time")
    track_nav_result_future = skill_manager.get_skill_data(skill_name, "track_nav_result_future")
    ...
    if track_nav_result_future is None:
        if track_nav_send_future.done():
            ...
            if goal_handle.accepted:
                ...
                skill_manager.set_skill_data(skill_name, "track_nav_result_future", track_nav_result_future)
                skill_manager.set_skill_data(skill_name, "track_nav_goal_handle", goal_handle)
    ...
    if track_nav_result_future.done():
        ...
        if result.status == GoalStatus.STATUS_SUCCEEDED and result.result.path.poses:
            ...
            skill_manager.set_skill_data(skill_name, "track_nav_move_start_time", robot.sim_time)
        ...
        elapsed = robot.sim_time - track_nav_start_time
        if elapsed > 15.0:
            track_nav_goal_handle = skill_manager.get_skill_data(skill_name, "track_nav_goal_handle")
...
def _handle_completed(robot, skill_manager, skill_name):
    ...
    skill_manager.set_skill_state(skill_name, "IDLE")  # Back to IDLE to check for next waypoint
    return skill_manager.form_feedback(
...
def _cancel_track(robot, skill_manager, skill_name):
    ...
    if robot.has_ros():
        track_waypoint_sub = skill_manager.get_skill_data(skill_name, "track_waypoint_sub")
