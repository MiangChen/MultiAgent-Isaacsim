...
def _init_move(robot, skill_manager, skill_name, kwargs):
    ...
    if not robot.has_ros():
        ...
        skill_manager.skill_errors[skill_name] = "ROS not available - movement requires ROS"
...
def _start_navigation_to_waypoint(robot, skill_manager, skill_name, goal_pos):
    ...
    skill_manager.set_skill_data(skill_name, "move_nav_send_future", move_nav_send_future)
...
def _handle_initializing(robot, skill_manager, skill_name):
    ...
    move_nav_send_future = skill_manager.get_skill_data(skill_name, "move_nav_send_future")
    move_nav_start_time = skill_manager.get_skill_data(skill_name, "move_nav_start_time")
    move_nav_result_future = skill_manager.get_skill_data(skill_name, "move_nav_result_future")
    ...
    if move_nav_result_future is None:
        if move_nav_send_future.done():
            ...
            if goal_handle.accepted:
                ...
                skill_manager.set_skill_data(skill_name, "move_nav_result_future", move_nav_result_future)
                skill_manager.set_skill_data(skill_name, "move_nav_goal_handle", goal_handle)
    ...
    if move_nav_result_future.done():
        ...
        if result.status == GoalStatus.STATUS_SUCCEEDED and result.result.path.poses:
            ...
            skill_manager.set_skill_data(skill_name, "move_nav_start_time", robot.sim_time)
        ...
        elapsed = robot.sim_time - move_nav_start_time
        if elapsed > 15.0:
            move_nav_goal_handle = skill_manager.get_skill_data(skill_name, "move_nav_goal_handle")
